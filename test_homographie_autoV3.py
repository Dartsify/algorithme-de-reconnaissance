...
def detect_outer_ellipse_points(image: np.ndarray) -> tuple[tuple[int, int], tuple[int, int], float, np.ndarray]:
	"""
	Détecte le grand bord externe de la cible, ajuste une ellipse et extrait
	quatre points extrêmes stables : haut, droite, bas, gauche.
	"""
	...
	contours = [contour for contour in contours if len(contour) >= 5 and cv2.contourArea(contour) >= image_area * 0.02]
	if not contours:
		raise RuntimeError("Impossible de détecter le contour externe de la cible.")

	contour_principal = max(contours, key=cv2.contourArea)
	ellipse = cv2.fitEllipse(contour_principal)
	...
	sampled_points = cv2.ellipse2Poly(center, axes, int(round(angle)), 0, 360, 1)
	if sampled_points.size == 0:
		raise RuntimeError("Impossible d'échantillonner l'ellipse détectée.")

	top_point = sampled_points[np.argmin(sampled_points[:, 1])]
	right_point = sampled_points[np.argmax(sampled_points[:, 0])]
	bottom_point = sampled_points[np.argmax(sampled_points[:, 1])]
	left_point = sampled_points[np.argmin(sampled_points[:, 0])]

	cardinal_points = np.array(
		[top_point, right_point, bottom_point, left_point],
		dtype=np.float32,
	)

	return center, axes, angle, cardinal_points
...
def main() -> None:
	...
	print(f"- Image avec caméra inclinée : {camera_image.shape[0]} x {camera_image.shape[1]} pixels")
	print(f"- Image de référence : {reference_image.shape[0]} x {reference_image.shape[1]} pixels")

	# Une homographie plane possède 8 degrés de liberté utiles, car la matrice
	# 3x3 est définie à un facteur d'échelle près. Il faut donc 4 correspondances
	# de points non alignés pour calculer la transformation.
	# Ici on automatise ces points en ajustant une ellipse sur le bord extérieur.
	src_center, src_axes, src_angle, pts_src = detect_outer_ellipse_points(camera_image)
	dst_center, dst_axes, dst_angle, pts_dst = detect_outer_ellipse_points(reference_image)

	print("\nEllipse détectée sur l'image inclinée :")
	print(f"- Centre : {src_center}")
	print(f"- Demi-axes : {src_axes}")
	print(f"- Angle : {src_angle:.2f}")

	print("\nEllipse détectée sur l'image de référence :")
	print(f"- Centre : {dst_center}")
	print(f"- Demi-axes : {dst_axes}")
	print(f"- Angle : {dst_angle:.2f}")

	print("\nPoints source (ellipse extérieure détectée) :")
	print(pts_src)
	print("\nPoints destination (ellipse extérieure détectée) :")
	print(pts_dst)
	...
	if homography_matrix is None:
		raise RuntimeError(
			...
			"ne sont pas alignés et que l'ellipse externe a bien été détectée."
		)
	...
	camera_image_with_points = draw_points(camera_image, pts_src, ["Haut", "Droite", "Bas", "Gauche"])
	camera_image_with_points = draw_points(camera_image_with_points,test_point_camera.reshape(1, 2),["Point test"])
	# cv2.imwrite(str(base_dir / "v3_cam1_reference_avec_points.jpg"), camera_image_with_points)

	reference_image_with_points = draw_points(reference_image, pts_dst, ["Haut", "Droite", "Bas", "Gauche"])
	# cv2.imwrite(str(base_dir / "v3_cam_reference_general_avec_points.jpg"), reference_image_with_points)
	...
	# cv2.imwrite(str(base_dir / "v3_cam1_homographie_avec_points.jpg"), corrected_image)
