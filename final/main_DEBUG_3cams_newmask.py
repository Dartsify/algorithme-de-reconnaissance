...
CAPTURE_DELAY_SECONDS = 0.8 # avant 0.5;  Temps entre la détection du mouvement et la capture des images (pour laisser le temps à la fléchette de se stabiliser)
CAPTURE_COOLDOWN_SECONDS = 1.2 # avant 1; Temps minimum entre deux captures pour éviter les faux positifs successifs
...
# rayon 10 : 300
# rayon 5 : 70
MIN_DART_CONTOUR_AREA = 100 # Adri: 100 ou 200 --- Seuil d'aire (en pixels) pour filtrer les contours de fléchettes valides avec les fausses détections
...
# TARGET_ID = os.getenv("DARTS_TARGET_ID", "190256")
TARGET_ID = os.getenv("DARTS_TARGET_ID", "190256")
...
def analyser_lancer(
    ...
    if point_camera is None or cam_utilisee is None:
        ...
        print(f"[INFO] Point original à l'échelle de la capture : ({point_original[0]:.2f}, {point_original[1]:.2f})")
...
def capturer_images(cameras):
    """Lit les images en séparant la capture matérielle du décodage logiciel."""
    frames = {}
    ...
    for camera_id, camera in cameras.items():
        ...
        if not success:
            raise RuntimeError(f"[ERREUR] Impossible d'initialiser l'image de la caméra {camera_id}.")
        frames[camera_id] = frame
...
def attendre_reprise_apres_pause(
	...
	while True:
		try:
			frames_actuelles = capturer_images(cameras)
...
def main() -> None:
	...
	print("Chargement des matrices d'homographie...")
	...
	frames_reference = capturer_images(cameras)
	previous_gray = {
	...
	while True:
		...
		if capture_en_attente and (maintenant - instant_detection) >= CAPTURE_DELAY_SECONDS:
			...
			nbLancer += 1
			print(f"[CHRONO] Choix de la caméra {meilleure_cam_id} parmi les 3 images : {(monotonic() - t_cap_start):.3f} secondes")
			...
			# derniere_capture = monotonic() # on met à jour après l'analyse pour inclure son temps dans le cooldown
